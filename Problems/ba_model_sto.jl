...
function ba_model_sto(name::AbstractString; T::Type = Float64)
    ...
    #r = zeros(length(sample))
  
    # nls functions #
    function resid!(
        rxs::AbstractVector,
        xs::AbstractVector;
        cam_indices::Vector{Int} = cams_indices,
        pnt_indices::Vector{Int} = pnts_indices,
        nobs::Int = nobs,
        npts::Int = npnts,
        ks::AbstractVector = ks,
        Ps::AbstractVector = Ps,
        pt2d::AbstractVector = pt2d,
        sample::AbstractVector = sample
      )
        @simd for i in eachindex(sample)
          cam_index = cam_indices[sample[i]]
          pnt_index = pnt_indices[sample[i]]
    ...
    projection!(x, c, r2, k, P1) = projection!(x, view(c, 1:3), view(c, 4:6), c[7], c[8], c[9], r2, k, P1)

...
function demo_solver_ba(nls, sampled_nls, h, χ, suffix="l0-linf"; n_runs = 1)
    ...
    x = [sol[3*i+1] for i in 0:(sampled_nls_test.nobs-1)]
    y = [sol[3*i+2] for i in 0:(sampled_nls_test.nobs-1)]
    z = [sol[3*i] for i in 1:sampled_nls_test.nobs]
    plt3d = plot(x, y, z, seriestype=:scatter)
    display(plt3d)
...
function demo_ba_sto(name::String; sample_rate = .05, n_runs = 1)
    ...
    demo_solver(nls, sampled_nls, h, χ, "lhalf-linf-$name"; n_runs = n_runs)
end
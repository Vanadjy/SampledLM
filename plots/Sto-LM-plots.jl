function plot_Sto_LM(sample_rates::AbstractVector, selected_probs::AbstractVector, selected_hs::AbstractVector; abscissa = "CPU time", n_exec = 10)
    compound = 1
    ...
    for selected_h in selected_hs
        ...
        if compare && (abscissa == "epoch")
            ...
            subsolver_options = RegularizedOptimization.ROSolverOptions(ϵa = 1e-3, ϵr = 1e-3, neg_tol = 1e-3, maxIter = 15)
            ...
            elseif param == "objective"
                plot!(1:k_R2, R2_out[:Fhist] + R2_out[:Hhist], label = "R2", lc = :red, ls = :dashdot, linetype=:steppre, xaxis = :log2, yaxis = yscale)
                plot!(1:length(LM_out.solver_specific[:Fhist]), LM_out.solver_specific[:Fhist] + LM_out.solver_specific[:Hhist], xaxis = :log2, yaxis = yscale, linetype=:steppre, label = "LM", lc = :orange, ls = :dot)
                plot!(1:length(LMTR_out.solver_specific[:Fhist]), LMTR_out.solver_specific[:Fhist] + LMTR_out.solver_specific[:Hhist], xaxis = :log2, linetype=:steppre, label = "LMTR", lc = :black, ls=:dash)
            end
        end
        ...
        local subsolver_options = RegularizedOptimization.ROSolverOptions(ϵa = 1e-3, ϵr = 1e-3, maxIter = 15)
        ...
        for (prob, prob_name) in nls_prob_collection
            ...
            for k in 1:n_exec
                ...
                if !probabilist
                    ...
                    prob.sample_rate = .05
                    SLM4_out = Prob_LM(prob, h, sampled_options; x0 = x0, subsolver_options = subsolver_options)
                end
                #reset!(prob)
            ...
            if abscissa == "epoch"
                ...
                if (param == "MSE") || (param == "accuracy") || (param == "objective")
                    if !guide
                        plot!(axes(Obj_Hists_epochs, 1), med_obj, color=color_scheme[sample_rate], lw = 1, linetype=:steppre, xaxis = :log2,  yaxis = yscale, label = "Sto_LM - $(sample_rate*100)%", title = "$prob_name on $n_exec runs - h = $h_name", ribbon=(std_obj, std_obj))
                    else
                        plot!(axes(Obj_Hists_epochs, 1), med_obj, color=color_scheme[sample_rate], lw = 1, linetype=:steppre, xaxis = :log2,  yaxis = yscale, label = "Sto_LM_guided - $(sample_rate*100)%", title = "$prob_name on $n_exec runs - h = $h_name", ribbon=(std_obj, std_obj))
                    end
                ...
                elseif param == "metric"
                    plot!(axes(Metr_Hists_epochs, 1), med_metric, color=color_scheme[sample_rate], lw = 1, linetype=:steppre, label = "Sto_LM - $(sample_rate*100)%", title = "Sampled √ξcp/νcp for $prob_name on $n_exec runs", xaxis=:log10, ribbon=(std_metric, std_metric))
                end

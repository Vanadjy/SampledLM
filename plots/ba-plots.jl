function plot_ba(name, sample_rates, versions; n_runs = 10, smooth::Bool = false, MaxEpochs::Int = 100)
    ...
    for sample_rate in sample_rates
        ...
        legend_obj_slm = PGFPlots.Plots.Linear(1:2, [1e16 for i in 1:2], mark = "$(symbols_cst_pgf[sample_rate])", style="$(color_scheme_pgf[sample_rate]), $(line_style_sto_pgf[sample_rate])", legendentry = "PLM $(legend_names_mnist[sample_rate])\\%")
        push!(data_obj, legend_obj_slm)
    end

    for version in versions
        med_obj_prob, med_metr_prob, med_mse_prob, std_obj_prob, std_metr_prob, std_mse_prob = load_ba_splm(name, version; n_runs = n_runs)#((name == "problem-16-22106-pre" && version == 2) ? 5 : n_runs))
        legend_obj_plm = PGFPlots.Plots.Linear(1:2, [1e16 for i in 1:2], mark = "$(symbols_nd_pgf[version])", style="$(prob_versions_colors_pgf[version]), $(line_style_plm_pgf[version])", legendentry = "PLM $(legend_names_mnist[version])")

...
for sample_rate in sample_rates
  ...
  sampled_options = ROSolverOptions(η3 = .4, ν = 1.0, νcp = 2.0, β = 1e16, ϵa = 1e-2, ϵr = 1e-2, verbose = 10;)
    ...
    for (h, h_name) ∈ ((NormL1(λ), "l1"),)
      ...
      @testset "bpdn-ls-Sto_LM_v4-$(h_name) - τ = $(sample_rate*100)%" begin
        ...
        if sample_rate == sample_rates[end-1]
          #title!(L"Evolution of $\sqrt{\frac{\hat{\xi}_{cp}}{\nu_{cp}}}$ through each epoch")
          title!("Evolution of metric through each epoch")
        end
